...
LibrettOS packages are based on [rumprun packages](https://github.com/rumpkernel/rumprun-packages). In this repository, we only include those packages that we used and/or modified. We also include additional packages (e.g., NFS) which were not part of the original repository.

Below are general instructions (from the original rumprun repository).
