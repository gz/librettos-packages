...
{
    cache_t *cache = cache_create("test", sizeof(uint64_t), sizeof(char*),
                                  NULL, NULL);
    assert(cache != NULL);
    cache_destroy(cache);
    return TEST_PASS;
...
static off_t flush_command(char* buf, size_t bufsz, uint8_t cmd, uint64_t exptime, bool use_extra) {
...
static enum test_return test_binary_get_impl(const char *key, uint8_t cmd) {
    ...
    uint64_t expiration = htonl(3600);
    size_t extlen = 0;
