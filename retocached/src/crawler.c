...
typedef void (*crawler_eval_func)(crawler_module_t *cm, item *it, uint64_t hv, int slab_cls);
...
static void crawler_expired_eval(crawler_module_t *cm, item *search, uint64_t hv, int i);
...
static void crawler_metadump_eval(crawler_module_t *cm, item *search, uint64_t hv, int i);
...
static void crawler_metadump_eval(crawler_module_t *cm, item *it, uint64_t hv, int i) {
...
static void *item_crawler_thread(void *arg) {
    ...
    while (crawler_count) {
        ...
        for (i = POWER_SMALLEST; i < LARGEST_ID; i++) {
            ...
            uint64_t hv = hash(ITEM_key(search), search->nkey);
 ...
 * LRU every time.
 */
static int do_lru_crawler_start(uint64_t id, uint64_t remaining) {
    uint64_t sid = id;
    int starts = 0;
    ...
    if (crawlers[sid].it_flags == 0) {
        if (settings.verbose > 2)
            fprintf(stderr, "Kicking LRU crawler off for LRU %lu\n", sid);
...
int lru_crawler_start(uint8_t *ids, uint64_t remaining,
