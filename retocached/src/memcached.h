...
// amounts of slabs allocated in one go 32MB, standard is 1MB
...
// Hashtable size = (1UL << HASHPOWER_DEFAULT) * sizeof(void *)      35 => 256GB  34 => 128GB
...
/// THE MAXIMUM AMOUNT OF SLABS for the ITEMS
#define BENCHMARK_SLAB_PREALLOC_SIZE \
    ((BENCHMARK_ITEM_VALUE_SIZE + 65)* BENCHMARK_MAX_KEYS) + \
    (((BENCHMARK_ITEM_VALUE_SIZE + 65)* BENCHMARK_MAX_KEYS) >> 3)

//// THE AMOUNT OF QUERIES PER THREAD
#define QUERIES_PER_THREAD (32UL * 1000UL * 1000UL * 1000UL)
...
// Hashtable size = (1UL << HASHPOWER_DEFAULT) * sizeof(void *)      35 => 256GB  34 => 128GB
#define HASHPOWER_DEFAULT 34
#define HASHPOWER_MAX 34

// for 1 TB use -> (1UL << 29)
#define BENCHMARK_MAX_KEYS (1UL << (HASHPOWER_MAX - 4))

//// THIS IS THE AMOUNT FOR THE HASH TABLE ITEM    128 ==>
#define BENCHMARK_ITEM_VALUE_SIZE (128 - 65)

/// THIS IS THE AMOUNT FOR THE ELEMENT ARRAY        128 ==>
#define BENCHMARK_ELEMENT_SIZE 128

/// THE MAXIMUM AMOUNT OF SLABS for the ITEMS
#define BENCHMARK_SLAB_PREALLOC_SIZE \
    ((BENCHMARK_ITEM_VALUE_SIZE + 65)* BENCHMARK_MAX_KEYS) + \
    (((BENCHMARK_ITEM_VALUE_SIZE + 65)* BENCHMARK_MAX_KEYS) >> 2)
...
//#define QUERIES_PER_THREAD (BENCHMARK_MAX_KEYS * 40UL)
...
#define FLAGS_CONV(iar, it, flag) { \
    if ((iar)) { \
        flag = (uint64_t) strtoul(ITEM_suffix((it)), (char **) NULL, 10); \
...
struct settings {
    ...
    uint64_t slab_chunk_size_max;  /* Upper end for chunks within slab pages. */
    ...
    uint64_t lru_crawler_tocrawl; /* Number of items to crawl per run */
    ...
    uint64_t temporary_ttl; /* temporary LRU threshold */
    int idle_timeout;       /* Number of seconds to let connections idle */
    uint64_t logger_watcher_buf_size; /* size of logger's per-watcher buffer */
    uint64_t logger_buf_size; /* size of per-thread logger buffer */
...
typedef struct {
    ...
    uint8_t         slabs_clsid;/* which slab class we're in */
    uint8_t         nkey;       /* key length, w/terminating null and padding */
    uint64_t        remaining;  /* Max keys to crawl per slab per invocation */
...
struct slab_rebalance {
    ...
    uint64_t busy_items;
    uint64_t rescues;
    uint64_t evictions_nomem;
    uint64_t inline_reclaim;
    uint64_t chunk_rescues;
    uint64_t busy_deletes;
    uint64_t busy_loops;
    uint8_t done;
};
...
enum store_item_type do_store_item(item *item, int comm, conn* c, const uint64_t hv);
...
item *item_touch(const char *key, const size_t nkey, uint64_t exptime, conn *c);
int   item_link(item *it);
void  item_remove(item *it);
int   item_replace(item *it, item *new_it, const uint64_t hv);
void  item_unlink(item *it);

void item_lock(uint64_t hv);
void *item_trylock(uint64_t hv);
void item_trylock_unlock(void *arg);
void item_unlock(uint64_t hv);
