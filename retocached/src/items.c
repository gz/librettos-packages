...
static bool lru_bump_async(lru_bump_buf *b, item *it, uint64_t hv);
...
unsigned int do_get_lru_size(uint64_t id) {
    return sizes[id];
}
...
int do_item_link(item *it, const uint64_t hv) {
...
int do_item_replace(item *it, item *new_it, const uint64_t hv) {
...
item *do_item_get(const char *key, const size_t nkey, const uint64_t hv, conn *c, const bool do_update) {
...
item *do_item_touch(const char *key, size_t nkey, uint64_t exptime,
                    const uint64_t hv, conn *c) {
