 ...
 * accordingly.
 */
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint64_t *slab_sizes) {
    int i = POWER_SMALLEST - 1;
    unsigned int size = sizeof(item) + settings.chunk_size;

    fprintf(stderr, "maximum %zu bytes (%zu GB) for the element slabs\n", limit, limit >> 30);
    ...
    if (prealloc) {
        ...
        printf("Preallocate the slabs");
        mem_base = calloc(mem_limit, 1);
        do_slab_prealloc = true;
#endif
#endif
        fprintf(stderr, "allocating %zu bytes (%zu GB) for the element slabs\n", limit, limit >> 30);
        mem_base = allocate(limit);
        //do_slab_prealloc = true;
        if (mem_base != NULL) {
...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            APPEND_NUM_STAT(i, "chunks_per_page", "%lu", perslab);
            APPEND_NUM_STAT(i, "total_pages", "%lu", slabs);
            APPEND_NUM_STAT(i, "total_chunks", "%lu", slabs * perslab);
            APPEND_NUM_STAT(i, "used_chunks", "%lu",
...
static void *memory_allocate(size_t size) {
    ...
    if (mem_base == NULL) {
        ...
        // if (remaining < size) {
        //     my_mem_base = allocate(16UL<<20);
        //     remaining = 16UL << 20;
        // }

        // ret = my_mem_base;
        // my_mem_base = (void *)((uintptr_t)my_mem_base + size);
        // remaining -= size;
