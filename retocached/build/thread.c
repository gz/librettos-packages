...
static uint64_t item_lock_count;
...
item *item_touch(const char *key, size_t nkey, uint64_t exptime, conn *c) {
    item *it;
    uint64_t hv;
    hv = hash(key, nkey);
    item_lock(hv);
 ...
 */
int item_replace(item *old_it, item *new_it, const uint64_t hv) {
