...
static void settings_init(void) {
    ...
    settings.inter = NULL;
    settings.maxbytes = BENCHMARK_SLAB_PREALLOC_SIZE; /* default is 64MB */
    ...
    settings.item_size_max =  USED_SLAB_PAGE_SIZE; /* The famous 1MB upper limit. */
    settings.slab_page_size =  USED_SLAB_PAGE_SIZE; /* chunks are split from 1MB pages. */
...
static void add_bin_header(conn *c, uint16_t err, uint8_t hdr_len, uint16_t key_len, uint64_t body_len) {
...
static void process_bin_get_or_touch(conn *c) {
    ...
    if (it) {
        ...
        uint16_t keylen = 0;
        uint64_t bodylen = sizeof(rsp->message.body) + (it->nbytes - 2);
...
static void append_bin_stats(const char *key, const uint16_t klen,
                             const char *val, const uint64_t vlen,
                             conn *c) {
    char *buf = c->stats.buffer + c->stats.offset;
    uint64_t bodylen = klen + vlen;
    protocol_binary_response_header header = {
...
static void dispatch_bin_command(conn *c) {
    ...
    uint64_t bodylen = c->binary_header.request.bodylen;
 ...
 */
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint64_t hv) {
        ...
        *p = ' ';
        ...
        if (it->nsuffix == 0) {
            *p = '0';
            ...
            p = itoa_u32(*((uint64_t *) ITEM_suffix(it)), p);
        }
        *p = ' ';
...
static inline item* limited_get(char *key, size_t nkey, conn *c, uint64_t exptime, bool should_touch) {
    item *it;
    if (should_touch) {
...
static void _get_extstore_cb(void *e, obj_io *io, int ret) {
    ...
    if (ret < 1) {
        ...
        uint64_t crc2;
        uint64_t crc = (uint64_t) read_it->exptime;
        int x;
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    if (! (safe_strtoul(tokens[2].value, (uint64_t *)&flags)
           && safe_strtol(tokens[3].value, &exptime_int)
           && safe_strtol(tokens[4].value, (uint64_t *)&vlen))) {
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
        ...
        if ((fp = fopen(pid_file, "r")) != NULL) {
            ...
            if (fgets(buffer, sizeof(buffer), fp) != NULL) {
                ...
                if (safe_strtoul(buffer, &pid) && kill((pid_t)pid, 0) == 0) {
                    fprintf(stderr, "WARNING: The pid file contained the following (running) pid: %lu\n", pid);
                }
            }
            fclose(fp);
        }
...
static bool _parse_slab_sizes(char *s, uint64_t *slab_sizes) {
    char *b = NULL;
    uint64_t size = 0;
    int i = 0;
    ...
    for (char *p = strtok_r(s, "-", &b);
        ...
        if (!safe_strtoul(p, &size) || size < settings.chunk_size
            ...
            fprintf(stderr, "slab size %lu is out of valid range\n", size);
            return false;
        }
        if (last_size >= size) {
            fprintf(stderr, "slab size %lu cannot be lower than or equal to a previous class size\n", size);
            return false;
        }
        if (size <= last_size + CHUNK_ALIGN_BYTES) {
            fprintf(stderr, "slab size %lu must be at least %d bytes larger than previous class\n",
...
{
  void *memptr;
  memptr = mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
  if (memptr == MAP_FAILED) {
    printf("ENOMEM: as tried to allocate %zu kB\n", size >> 10);
    exit(1);
  }
...
int main (int argc, char **argv) {
    ...
    uint64_t tocrawl;
    uint64_t slab_sizes[MAX_NUMBER_OF_SLAB_CLASSES];
    ...
    if (settings.slab_chunk_size_max > settings.item_size_max) {
        fprintf(stderr, "slab_chunk_max (bytes: %ld) cannot be larger than -I (item_size_max %d)\n",
                settings.slab_chunk_size_max, settings.item_size_max);
        exit(EX_USAGE);
    }

    if (settings.item_size_max % settings.slab_chunk_size_max != 0) {
        fprintf(stderr, "-I (item_size_max: %d) must be evenly divisible by slab_chunk_max (bytes: %ld)\n",
                settings.item_size_max, settings.slab_chunk_size_max);
        exit(EX_USAGE);
    }

    if (settings.slab_page_size % settings.slab_chunk_size_max != 0) {
        fprintf(stderr, "slab_chunk_max (bytes: %ld) must divide evenly into %d (slab_page_size)\n",
                settings.slab_chunk_size_max, settings.slab_page_size);
        exit(EX_USAGE);
    }
#ifdef EXTSTORE
...
//    }
    ...
    if (settings.socketpath == NULL && false) {
...
#define KEY_LENGTH 26
  ...
  for (size_t i = 0; i < 256; i++) {
   ...
   struct element *elms = allocate(BENCHMARK_MAX_KEYS * sizeof(struct element));
...
//            omp_get_num_threads(),thread_id, (void *)myconn);
  ...
  printf("%f,,cycles,0,0,0\n", sec);
  printf("0,,dtlb_store_misses.walk_duration,0,0,0,M/sec\n");
  printf("0,,dtlb_load_misses.walk_duration,0,0,0,M/sec\n");
